...
int evaluate(Board *brd, int player)
{
    ...
    {
        int positionalScore = playerPositionalScore - opponentPositionalScore;
        int bomberScore = playerBomberCount - opponentBomberCount;
        int stingerScore = playerStingerCount - opponentStingerCount;
        int evaluation = positionalScore + (9 * bomberScore + stingerScore);
        return evaluation;
    }
}
...
int main()
{
    ...
    for(int i = 0; i < 5; i++)
    {
        for(int j = 0; j < 5; j++)
        {
            ...
            {
                stingerWeights[i][j] = 3;
                bomberWeights[i][j] = 5;
            }
            if(i == (myBase + (myMultiplier * 2)))
            {
                stingerWeights[i][j] = 4;
                bomberWeights[i][j] = 11;
            }
            if(i == (myBase + (myMultiplier * 3)))
            {
                if(j == 0 || j == 4)
                {
                	stingerWeights[i][j] = 6;
                	bomberWeights[i][j] = 14;
                }
                else   
                {
                	stingerWeights[i][j] = 4;
                	bomberWeights[i][j] = 12;
                }
            }
            if(i == (myBase + (myMultiplier * 4)))
            {
                stingerWeights[i][j] = -2;
                bomberWeights[i][j] = INF;
            }
            if(i == opponentBase)
                opponentBomberWeights[i][j] = 1;
            if(i == (opponentBase + (opponentMultiplier * 1)))
            {
                opponentStingerWeights[i][j] = 3;
                opponentBomberWeights[i][j] = 5;
            }
            if(i == (opponentBase + (opponentMultiplier * 2)))
            {
                opponentStingerWeights[i][j] = 4;
                opponentBomberWeights[i][j] = 11;
            }
            if(i == (opponentBase + (opponentMultiplier * 3)))
            {
                if(j == 0 || j == 4)
                {
                    opponentStingerWeights[i][j] = 6;
                    opponentBomberWeights[i][j] = 14;
                }
                else
                {
                    opponentStingerWeights[i][j] = 4;
                    opponentBomberWeights[i][j] = 12;
                }
            }
            if(i == (opponentBase + (opponentMultiplier * 4)))
            {
                opponentStingerWeights[i][j] = -2;
                opponentBomberWeights[i][j] = INF;
            }
        }
    }
	while(1)
	{
        ...
        std::cout<<bestMove.x1<<" "<<bestMove.y1<<" "<<bestMove.x2<<" "<<bestMove.y2<<"\n";
	}
    return 0;
}
