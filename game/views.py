...
class GridView(View):
    ...
    def get(self, request):
        ...
        return render(request, self.template_name, {'au': au, 'username': request.user.username, 'pro': pro,'opp':41})

    def post(self, request):
        pro = User.objects.all()
        au = request.user.is_authenticated
        if not request.user.is_authenticated:
            return render(request, self.template_name, {'suc': 'You must log in to upload a file', 'au': au, 'pro': pro,
                                                        'username': request.user.username, 'opp':41})
    ...
    def bot_upload(self, request):
        ...
        if request.FILES:
            ...
            if result == 0:
                ...
                return render(request, self.template_name,
                              {'suc': 'File should be a C, CPP or Python file', 'pro': pro, 'au': au,
                               'username': request.user.username,'opp':41})
            try:
                curr_obj.userprofile.save()
            except:
                return render(request, self.template_name,
                              {'suc': 'Upload Unsuccessful! Please Try Again', 'au': au, 'pro': pro,
                               'username': request.user.username,'opp':41})
            return render(request, self.template_name,
                          {'suc': 'Upload Successful!', 'au': au, 'pro': pro, 'username': request.user.username,'opp':41})
        else:
            return render(request, self.template_name,
                          {'au': au, 'suc': 'Please upload a file', 'pro': pro, 'username': request.user.username,'opp':41})
    ...
    def match_request(self, request):
        ...
        if not opp_id:
            return render(request, self.template_name,
                          {'suc': 'Please Select an opponent', 'au': au, 'pro': pro, 'username': request.user.username,'opp':41})
        my_id = curr_obj.pk
        # if not p2flag:
        ...
        for match_temp in request_queue:
            if match_temp.identifier_string == match:
                return render(request, self.template_name, {'suc': 'Match is already in queue', 'au': au, 'pro': pro,
        ...
        if result is not None:
            print(result)
            return render(request, self.template_name,
                          {'suc': result, 'au': au, 'pro': pro, 'username': request.user.username,'opp': int(opp_id)})
        ...
        return render(request, self.template_name,
                      {'suc': 'Match requested!', 'au': au, 'pro': pro, 'username': request.user.username,'opp': int(opp_id)})
    ...
    def view_request(self, request):
        ...
        if not opp_id:
            return render(request, self.template_name,
                          {'suc': 'Please select opponent', 'au': au, 'pro': pro, 'username': request.user.username,'opp':41})
        p2flag = request.POST.get('p2flag')
        if not p2flag:
            match = str(my_id) + 'v' + opp_id
            reverse_match = opp_id + 'v' + str(my_id)
        ...
        if not os.path.isfile(error_path):
            ...
            if not os.path.isfile(rev_error_path):
                print('wrong')
                return render(request, self.template_name, {'suc': 'Match not processed yet', 'au': au, 'pro': pro,
                                                            'username': request.user.username,'opp':int(opp_id)})
        # else:
        ...
        print error_path
        return render(request, self.template_name,
                      {'au': au, 'log': log_pass, 'error': error_pass, 'pro': pro, 'username': request.user.username,'opp':int(opp_id)})
